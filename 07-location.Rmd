...
- Where are competiting stores or services located?
...
The `census` object is a data frame containing 13 variables for more than 300,000 grid cells accross Germany.
...
This is done by first using the x and y coordinates in the `census` data frame into a spatial object.
...
```{r}
# convert create sf version (x and y are cell midpoints), set CRS
input_sf = st_as_sf(input_tidy, coords = c("x", "y"), crs = 3035)
# convert into an sp-object
input_sp = as(input_sf, "Spatial")
gridded(input_sp) = TRUE
# convert into a raster stack
input_ras = stack(input_sp)
# ras = rasterFromXYZ(input_tidy[1:3], crs = 3035)
# s = lapply(3:6, function(x) rasterFromXYZ(input_tidy[c(1:2, x)], crs = 3035) )
# stack(s)
# ras$women = input_tidy$women
```
